...
//#include "ThirdParty/lzokay/lzokay.hpp"
...
fs::path get_user_settings_dir(GameVersion version) {
  fs::path filepath;
  if (version == GameVersion::Sly1 || version == GameVersion::Sly2) {
      filepath = get_user_config_dir() / game_version_names[version] / "settings";
  }
  else {
      filepath = get_user_config_dir() / "sly2" / "settings";
  }

  return filepath;
}

fs::path get_user_memcard_dir(GameVersion version) {
    fs::path filepath;
    if (version == GameVersion::Sly1 || version == GameVersion::Sly2) {
        filepath = get_user_config_dir() / game_version_names[version] / "saves";
    }
    else {
        filepath = get_user_config_dir() / "sly2" / "saves";
    }
...
/*!
 * See if the current executable is somewhere in Sly2Decomp/. If so, return the path to Sly2Decomp
 */
std::optional<std::string> try_get_sly_project_path() {
  std::string my_path = get_current_executable_path();

  //FIXME: Change File path
  std::string::size_type pos =
      std::string(my_path).rfind("Sly2Decomp");  // Strip file path down to Sly2Decomp directory
  if (pos == std::string::npos) {
    return {};
  }

  return std::make_optional(std::string(my_path).substr(
      0, pos + 11));  // + 12 to include "Sly2Decomp" in the returned filepath
}
...
bool setup_project_path(std::optional<fs::path> project_path_override) {
  ...
  auto development_repo_path = try_get_sly_project_path();
...
std::string get_file_path(const std::vector<std::string>& input) {
  ...
  auto current_path = file_util::get_sly_project_dir();
  for (auto& str : input) {
    current_path /= str;
  }
