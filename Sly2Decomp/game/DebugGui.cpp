...
void FrameTimeRecorder::draw_window(const DmaStats&) {
  auto* p_open = &m_open;
...
void OpenGlDebugGui::draw(const DmaStats& dma_stats) {
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu("Frame Rate")) {
      ImGui::Checkbox("Enable V-Sync", &Graphics::g_global_settings.IsVsyncEnabled);
      ImGui::Separator();
      ImGui::Checkbox("Framelimiter", &Graphics::g_global_settings.IsFrameLimiterEnabled);
      ImGui::InputFloat("Target FPS", &target_fps_input);
      if (ImGui::MenuItem("Apply")) {
        Graphics::g_global_settings.TargetFps = target_fps_input;
      }
    }
    ...
    if (ImGui::BeginMenu(fmt::format("WORK IN PROGRESS VERSION!").c_str())) {
      ImGui::EndMenu();
    }
  }
